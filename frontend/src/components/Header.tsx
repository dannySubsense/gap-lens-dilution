...
function RiskBadgeInline({ level }: { level: string }) {
  ...
  return (
    <span
      className={`${colorMap[level] ?? "bg-badge-default"} text-white text-meta font-bold px-3 py-1 rounded-[var(--radius-sm)]`}
    >
      RISK: {level}
    </span>
  );
  ...
  return (
    <div className="bg-bg-card border border-border-card rounded-[var(--radius)] p-5">
      ...
      <div className="flex items-center gap-3 mb-2 flex-wrap">
        <h1 className="text-section font-bold text-accent-purple">{data.ticker}</h1>
      ...
      {data.stockPrice !== null && data.stockPrice > 0 && (
        <p className="text-meta font-bold text-text-primary mb-2">

    ...
    <div className="bg-bg-card border border-border-card rounded-[var(--radius)] p-5">
      ...
      <div className="max-h-96 overflow-y-auto space-y-4 pr-1">
        {data.map((note, i) => (
          ...
          >
            <p className="text-text-muted text-meta font-[JetBrains_Mono,ui-monospace,monospace] mb-1">
              {note.date}
            </p>
            <p className="text-text-primary text-meta font-bold mb-1">
              {note.ticker}
            </p>
            {note.content.map((line, j) => (
              <p key={j} className="text-text-secondary text-meta leading-relaxed">
                {line}
              </p>
            ))}
          </div>
        ...
        {data.length === 0 && (
          <p className="text-text-muted text-meta">No notes available.</p>
        )}
      </div>
    </div>
  );
